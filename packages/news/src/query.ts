...
/**
 * Internal-only product query. Do not expose via public HTTP while
 * SCOOP_NEWS_PUBLIC_DISPLAY_ENABLED is false.
 */
export async function getLatestNews(
  ...
  const result = await db.query<Row>(
    `SELECT provider_article_id, title, description, source_domain, url,
     ...
     ORDER BY provider_crawled_at DESC, provider_article_id DESC
     LIMIT ${limitParam}`,
    params,
  );
