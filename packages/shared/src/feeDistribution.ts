...
export function normalizeFeeDistributionArgs(
  ...
  if (isCanonicalDistributionArgs(args)) {
    ...
    return {
      ...
      extraDeployerRaw,
      extraHoldersRaw,
      creatorRaw: baseCreatorRaw + extraCreatorRaw,
      deployerRaw: baseDeployerRaw + extraDeployerRaw,
      buybackRaw: baseProtocolRaw,
      operationsRaw: baseOperationsRaw,
      holdersRaw: baseHoldersRaw + extraHoldersRaw,
    };
  }
...
/** Exact conservation: sum of base+extra legs == totalRaw. */
export function assertDistributionConservation(
  ...
  if (sum !== legs.totalRaw) {
    throw new Error(
      `fee distribution conservation failed: sum=${sum.toString()} total=${legs.totalRaw.toString()}`,
    );
  }
}

export function distributionConserves(legs: NormalizedFeeDistributionLegs): boolean {
  try {
