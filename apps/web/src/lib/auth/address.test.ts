...
describe('address normalization', () => {
  ...
  it('compares Solana base58 exactly and never via EVM normalization', () => {
    expect(addressesEqual(SOL, SOL)).toBe(false);
    expect(walletIdentitiesEqual(SOL, SOL)).toBe(true);
    expect(walletIdentitiesEqual(SOL, SOL_OTHER)).toBe(false);
    expect(walletIdentitiesEqual(LOWER, SOL)).toBe(false);
    expect(walletIdentitiesEqual(LOWER, CHECKSUM)).toBe(true);
  });
});
